...
type SummaryRecord = Record<string, unknown> & { outputs?: Record<string, unknown> };
type ToolTextResponse = { content: Array<{ type: 'text'; text: string }> };
type ProcessOptions = { timeoutMs?: number; cwd?: string };
...
const executableCache = new Map<string, ProbeResult>();
...
function text(payload: unknown): ToolTextResponse {
  return { content: [{ type: 'text', text: JSON.stringify(payload, null, 2) }] };
}

function fail(tool: string, error: unknown, extra: Record<string, unknown> = {}): ToolTextResponse {
  return text({ success: false, tool, error: error instanceof Error ? error.message : String(error), ...extra });
}

function bind(depKey: string, methodName: string) {
  return (deps: Record<string, any>) => async (args: AnyArgs = {}) => deps[depKey][methodName](args ?? {});
}

function stringArg(args: AnyArgs | undefined, key: string, required = false): string | undefined {
...
function intArg(args: AnyArgs | undefined, key: string, fallback: number): number {
  const value = args?.[key];
...
function stringArrayArg(args: AnyArgs | undefined, key: string): string[] {
  const value = args?.[key];
...
async function ensureFile(targetPath: string, label: string): Promise<string> {
  const absolutePath = resolve(targetPath);
  const fileStat = await stat(absolutePath).catch(() => null);
  if (!fileStat) throw new Error(`${label} does not exist: ${absolutePath}`);
  if (!fileStat.isFile()) throw new Error(`${label} is not a file: ${absolutePath}`);
  return absolutePath;
}

async function ensureInputDir(targetPath: string, label: string): Promise<string> {
  const absolutePath = resolve(targetPath);
  const dirStat = await stat(absolutePath).catch(() => null);
  if (!dirStat) throw new Error(`${label} does not exist: ${absolutePath}`);
  if (!dirStat.isDirectory()) throw new Error(`${label} is not a directory: ${absolutePath}`);
  return absolutePath;
}

async function nextDir(baseDir: string): Promise<string> {
...
async function resolveManagedStage(inputPath, stageName, requestedOutputDir, requestedWorkDir) {
  if (requestedOutputDir) {
    ...
    if (stageStat && !stageStat.isDirectory()) throw new Error(`outputDir is not a directory: ${stageDir}`);
    if (stageStat) {
      const entries = await readdir(stageDir);
      if (entries.length > 0) throw new Error(`outputDir already exists and is not empty: ${stageDir}`);
    }
...
async function writeSummary(analysisRoot: string, patch: SummaryRecord): Promise<string> {
  await ensureDir(analysisRoot);
  const summaryPath = join(analysisRoot, 'summary.json');
  let base: SummaryRecord = { pluginId: PLUGIN_ID, createdAt: new Date().toISOString() };
  if (await exists(summaryPath)) {
    try {
      const parsed = JSON.parse(await readFile(summaryPath, 'utf-8'));
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) base = parsed as SummaryRecord;
    } catch {
    }
  }
  const merged = {
...
function bundledPlatformKey(): string | null {
  if (process.platform === 'win32' && process.arch === 'x64') return 'windows-x64';
  return null;
}

function bundledPlatformRoot(): string | null {
  const key = bundledPlatformKey();
  return key ? join(bundledToolsRoot, key) : null;
}

function bundledCommandCandidates(toolName: string): string[] {
  const platformRoot = bundledPlatformRoot();
  if (!platformRoot) return [];
  if (toolName === 'java') return [join(platformRoot, 'jre', 'bin', process.platform === 'win32' ? 'java.exe' : 'java')];
  if (toolName === 'apktool') return [join(platformRoot, 'apktool', process.platform === 'win32' ? 'apktool.cmd' : 'apktool')];
  if (toolName === 'jadx') return [process.platform === 'win32' ? join(platformRoot, 'jadx.cmd') : join(platformRoot, 'jadx', 'bin', 'jadx')];
  return [];
}

function commandCandidates(toolName: string): string[] {
  const bundled = bundledCommandCandidates(toolName);
  if (toolName === 'java') return [...bundled, 'java'];
  if (toolName === 'apktool') return process.platform === 'win32' ? [...bundled, 'apktool', 'apktool.bat'] : [...bundled, 'apktool'];
  if (toolName === 'jadx') return process.platform === 'win32' ? [...bundled, 'jadx', 'jadx.bat'] : [...bundled, 'jadx'];
  return [...bundled, toolName];
}

function versionArgs(toolName: string): string[] {
...
async function runProcess(command: string, args: string[], options: ProcessOptions = {}): Promise<RunProcessResult> {
  const timeoutMs = options.timeoutMs ?? 300000;
  return await new Promise<RunProcessResult>((resolvePromise) => {
...
async function probe(toolName: string): Promise<ProbeResult> {
...
async function needTool(toolName: string): Promise<string> {
  const found = await probe(toolName);
  if (!found.available) throw new Error(`${toolName} is not available: ${found.reason ?? 'unknown error'}`);
  return found.command;
}

async function needApk(args) {
  const inputPath = await ensureFile(stringArg(args, 'inputPath', true), 'inputPath');
  if (extname(inputPath).toLowerCase() !== '.apk') throw new Error(`inputPath must point to an .apk file: ${inputPath}`);
  return inputPath;
}
function gather(source, regex) {
  const results = [];
  for (const match of source.matchAll(regex)) {
    if (typeof match[1] === 'string' && match[1].trim()) results.push(match[1].trim());
  }
  return results;
}
...
async function latestStage(analysisRoot, prefix) {
  if (!(await exists(analysisRoot))) return null;
...
function searchable(filePath, scope) {
  ...
  if (scope === 'smali') return extension === '.smali';
...
function snippet(line, query) {
  ...
  if (index < 0) return line.trim().slice(0, 240);
  ...
  return {
    ...
    async handleManifestSummary(args) {
      try {
        ...
        if (unpackDirArg) {
          ...
          if (!unpackDir) throw new Error(`No managed apktool output found for ${resolve(inputPathArg)}. Run apk_unpack first or pass unpackDir.`);
        } else {
    ...
    async handleSearchCode(args) {
      try {
        ...
        if (!['java', 'smali', 'all'].includes(scope)) throw new Error('scope must be one of: java, smali, all');
        ...
        if (scope === 'java' || scope === 'all') {
          if (decompileDirArg) {
            ...
            if (stage) roots.push({ scope: 'java', root: await javaRoot(stage) });
            else warnings.push('No managed JADX output found. Run apk_decompile first or pass decompileDir.');
          }
        }
        if (scope === 'smali' || scope === 'all') {
          if (!inputPathArg) {
            ...
            if (stage) {
              for (const root of await smaliRoots(stage)) roots.push({ scope: 'smali', root });
            } else {
        ...
        if (uniqueRoots.length === 0) throw new Error(`No searchable roots found for scope=${scope}`);
        ...
        for (const item of uniqueRoots) {
          await walkFiles(item.root, async (filePath) => {
            ...
            if (!searchable(filePath, item.scope)) return;
